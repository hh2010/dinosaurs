...
try:
    ...
    df = pd.read_csv('data/pbdb_data.csv', 
                     skiprows=header_row,
                     encoding='utf-8',
    ...
    taxonomic_columns = [
        'identified_name', 'identified_rank', 'accepted_name', 'accepted_rank',
    ...
    columns_to_show = taxonomic_columns + discovery_columns + ['early_interval', 'late_interval', 'max_ma', 'min_ma']
    ...
    trex_mask = df[taxonomic_columns].apply(lambda x: x.apply(contains_trex)).any(axis=1)
    ...
    if not trex_records.empty:
        ...
        for idx, record in trex_records.iterrows():
            ...
            logging.info("\nDiscovery Information:")
            logging.info(f"Location: {record['collection_name']} ({record['cc']}, {record['state']})")
    ...
    def create_fossil_map(records):
        try:
            ...
            # Save map
            map_path = os.path.join('data', 'trex_locations.html')
            m.save(map_path)
    ...
    def create_discovery_timeline(records):
        try:
            ...
            # Save plot
            timeline_path = os.path.join('data', 'trex_timeline.png')
    ...
    output_path = os.path.join('data', 'trex_analysis.csv')
